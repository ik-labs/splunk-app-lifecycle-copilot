...
**Self-heal engine.** A single, artifact-agnostic loop — `diagnose → select deterministic patch → apply → re-validate → repeat` (capped by `SELF_HEAL_MAX_ITERS`). The same engine drives both loops; only the diagnosis source and patch registry differ. This reuse is the platform thesis.
